...
from .layers.multimodal_decoder import  MoE_QueryDecoder, MultimodalDecoder, QueryBasedMoeDecoder, RegressionSegmentDecoder, RegressionSegmentDecoder_v2, SimpleSegmentalMoeDecoder,HierarchicalGatingDecoder
...
class MoeMotion_new(nn.Module):
    def __init__(
        ...
        if moe:
            if moe_type == "cross":
                ...
                self.decoder = RegressionSegmentDecoder_v2(
                    embed_dim=embed_dim,
                    num_modes=modes,
                    future_len=future_len,
                    future_steps=future_steps,
    ...
    def forward(self, data, mode):
        ...
        actor_feat = actor_feat.permute(0, 2, 1, 3).reshape(-1, N, actor_feat.shape[-1])
        hist_mask = hist_key_valid_mask.unsqueeze(1).repeat(1, segment, 1).reshape(-1, N)
        for blk in self.agent_blocks:
            actor_feat = blk(actor_feat,key_padding_mask=~hist_mask)
        ...
        x_centers_agents = data['x_centers'].unsqueeze(2).repeat(1, 1, segment, 1)  # [B, N, segment, 2]
        x_centers = x_centers_agents.reshape(B, N*segment, -1)
        x_angles_agents = data['x_angles'][:, :, -1].unsqueeze(2).repeat(1, 1, segment).reshape(B, N*segment)  # [B, N, segment, 1]
        x_angles = torch.stack([torch.cos(x_angles_agents), torch.sin(x_angles_agents)], dim=-1)
        x_pos_feat = torch.cat([x_centers, x_angles], dim=-1)
        x_pos_embed = self.pos_embed(x_pos_feat)

        lane_centers = data['lane_centers']
        lane_angles = data['lane_angles']
        lane_angles = torch.stack([torch.cos(lane_angles), torch.sin(lane_angles)], dim=-1)
        lane_pos_feat = torch.cat([lane_centers, lane_angles], dim=-1)
        lane_pos_embed = self.pos_embed(lane_pos_feat)

        lane_feat += lane_pos_embed
        actor_feat += x_pos_embed

        lane_mask = data['lane_key_valid_mask']
        for blk in self.map_blocks:
            lane_feat = blk(lane_feat, key_padding_mask=~lane_mask)

        actor_mask = data['x_key_valid_mask'].unsqueeze(-1).repeat(1, 1, segment).reshape(B, -1)
        for blk in self.blocks:
            actor_feat = blk(src=actor_feat,src_kv=lane_feat, key_padding_mask=~lane_mask)
        
        if self.moe:
            y_hat = self.decoder(actor_feat,lane_feat, actor_mask=~actor_mask, lane_mask=~lane_mask,his_seg=segment)
        else:
            x_agent = actor_feat[:, :segment]
            y_hat = self.decoder(x_agent)
        x_others = actor_feat[:, segment:segment*(N), :]
        x_others = x_others.reshape(B, N-1,segment, -1) 
        x_others = x_others.reshape(B, N-1, -1)
        y_hat_others = self.dense_predictor(x_others).view(B, x_others.size(1), -1, 2)
        ret_dict = {
