...
class RegressionSegmentDecoder_v2(nn.Module):
    def __init__(self,
        ...
        self.history_norm = nn.LayerNorm(embed_dim)
        self.context_norm = nn.LayerNorm(embed_dim)
        self.query_init =nn.ModuleList(Inter_map_hist_Block(
                    dim=embed_dim,
                    num_heads=num_heads,
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
    ...
    def forward(self, actor_feat,lane_feat, actor_mask=None, lane_mask=None,his_seg=1):
        ...
        for blk in self.query_init:
            initial_state = blk(src=queries, hist_feat=actor_feat, map_feat=lane_feat,
                                hist_mask=actor_mask, map_mask=lane_mask)
        ...
        for i in range(self.num_segments):
            thought_state = self.gru_cell(gru_input, thought_state)
            thought_state_q = thought_state.view(B, self.num_modes, self.embed_dim)
            context_output = self.query_attn[i](src=thought_state_q, src_kv=lane_feat,
