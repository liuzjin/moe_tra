...
from visual.viz import (
    visualize_scenario,
    visualize_scenario_last_frame,
)
...
def  generate_scenario_visualizations(
    ...
    *,
    ...
    def generate_scenario_visualization(scenario_path: Path, last: bool = False) -> None:
        ...
        if last:
            viz_save_path = viz_output_dir / f"{scenario_id}_last.png"
        else:
        ...
        if last:
            visualize_scenario_last_frame(scenario, static_map, viz_save_path)
        else:
    ...
    if debug:
        for scenario_path in track(scenario_file_list):
            generate_scenario_visualization(scenario_path, last)
    else:
        Parallel(n_jobs=_DEFAULT_N_JOBS)(
            delayed(generate_scenario_visualization)(scenario_path, last)
            for scenario_path in track(scenario_file_list)
        )
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Generate visualizations for the last frame of each scenario when enabled.",
)
